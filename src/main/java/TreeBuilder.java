...
public class TreeBuilder {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (splitListSize > currentIndex) {
            ...
            if(entropy!=0.0 && currentSplit.att_index.size()!=AttributeNumber){
                bestGainRatio=0;
                for(int i=0;i<AttributeNumber;i++){		//Finding the gain of each attribute
                    if(!currentSplit.att_index.contains(i)){ //表示这个属性在当前节点下属还木有被分裂过
                ...
                for(int i=1; i<=splitNumber; i++){
                    newSplit=new Split();
                    for(int j=0;j<currentSplit.att_index.size();j++){

                        newSplit.att_index.add(currentSplit.att_index.get(j));
                        newSplit.att_value.add(currentSplit.att_value.get(j));
                    }
                    newSplit.att_index.add(attributeIndex);
                    newSplit.att_value.add(attributes.nextToken());
                    splitList.add(newSplit);
                }
            }else{
                String rule="";
                for(int i=0;i<currentSplit.att_index.size();i++){
                    rule=rule+" "+ currentSplit.att_index.get(i)+" "+ currentSplit.att_value.get(i);
                }
